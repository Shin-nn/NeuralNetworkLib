...
namespace BasisFunction
{
	class Radial: public BasisFunction
	{
		public:
			...
			virtual float operator()(const Tensor::Tensor<float>& weights, const Tensor::Tensor<float>& input) const override {
				float sum = 0.0;
				for(std::size_t i=0;i<weights.size().size();i++) {
					sum+=pow(input.data()[i]-weights.data()[i],2);
				}
				return sqrt(sum);
			}
