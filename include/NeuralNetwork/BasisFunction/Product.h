...
namespace BasisFunction {
	...
	class Product: public BasisFunction {
		public:
			 ...
			 */
			inline virtual float operator()(const Tensor::Tensor<float>& weights, const Tensor::Tensor<float>& input) const override {
				float product=1.0;
				for(size_t i=0;i<weights.size().size();i++) {
					if(weights.data()[i] > 0.5)
						product=product*input.data()[i];
				}
				return product;
			}
